...
<section>
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <h3 class="text-center mb-4">{{ trans('file.Monthly Purchase Report') }}</h3>

                <div class="filter-card">
                    {{ Form::open(['url' => 'report/monthly_purchase/' . $year, 'method' => 'post', 'id' => 'report-form']) }}
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-bordered" style="border-top: 1px solid #dee2e6; border-bottom: 1px solid #dee2e6;">
                        <thead>
                            <tr class="bg-light">
                                <th>
                                    <a href="{{ url('report/monthly_purchase/' . ($year - 1)) }}" class="btn btn-outline-secondary btn-sm prev-next-link" data-year="{{ $year - 1 }}">
                        ...
                        <tbody>
                            ...
                            <tr>
                                @for ($m = 1; $m <= 12; $m++)
                                    <td>
                                        @if ((isset($grand_total[$m]) && $grand_total[$m] > 0) || (isset($total_purchase[$m]) && $total_purchase[$m] > 0))
                                            @php
                                            ...
                                            <a href="{{ route('purchases.index', ['starting_date' => $start_date, 'ending_date' => $end_date]) }}"
                                                ...
                                                {{-- Total Purchase --}}
                                                @if (isset($total_purchase[$m]) && $total_purchase[$m] > 0)
                                                    <strong>Total : </strong> <span>{{ $total_purchase[$m] }}</span><br>
                                                @endif
                ...
                {{-- Yearly Purchase Summary Calculations --}}
                @php
                    $year_total_purchase = isset($total_purchase) ? array_sum($total_purchase) : array_sum($grand_total);
                    $year_total_return = isset($total_return) ? array_sum($total_return) : 0;
                                        ...
                                        <table class="table table-sm table-bordered mb-0">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="font-weight-bold text-right">{{ number_format($year_total_purchase, 2) }}</td>
                                                </tr>
...
<script type="text/javascript">
    ...
    $('#year_picker').datepicker({
        ...
        if(e.date) {
            ...
            submitMonthlyPurchaseFilter(currentYear);
        }
    });

    function submitMonthlyPurchaseFilter(y) {
        var selected_year = y || currentYear;
        var action_url = "{{ url('report/monthly_purchase') }}/" + selected_year;
