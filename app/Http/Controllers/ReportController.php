...
class ReportController extends Controller
{
    ...
    public function dailySaleByWarehouse(Request $request, $year, $month)
    {
        ...
        if ($data['warehouse_id'] == 0)
            return redirect('report/daily_sale/' . $year . '/' . $month);
        $start = 1;
        $number_of_day = date('t', mktime(0, 0, 0, $month, 1, $year));
        while ($start <= $number_of_day) {
            if ($start < 10)
    ...
    public function dailyPurchaseByWarehouse(Request $request, $year, $month)
    {
        ...
        while ($start <= $number_of_day) {
            ...
            $purchase_data = Purchase::with('productPurchases.product.brand')
                ...
                ->whereDate('created_at', $date)
                ->get();
            $return_data = ReturnPurchase::where('warehouse_id', $data['warehouse_id'])
                ->whereDate('created_at', $date)
                ->get();
    ...
    public function monthlySaleByWarehouse(Request $request, $year)
    {
        $data = $request->all();
        if ($data['warehouse_id'] == 0)
            return redirect('report/monthly_sale/' . $year);
    ...
    public function monthlyPurchaseByWarehouse(Request $request, $year)
    {
        $data = $request->all();
        if ($data['warehouse_id'] == 0)
            return redirect('report/monthly_purchase/' . $year);
    ...
    {
        return $this->bestSeller($request);
    }
    ...
    //     return view('backend.report.cash', compact('purchase', 'product_cost', 'product_tax', 'total_purchase', 'sale', 'total_sale', 'return', 'purchase_return', 'total_return', 'total_purchase_return', 'expense', 'payroll', 'total_expense', 'total_payroll', 'payment_recieved', 'payment_recieved_number', 'cash_payment_sale', 'cheque_payment_sale', 'credit_card_payment_sale', 'gift_card_payment_sale', 'paypal_payment_sale', 'deposit_payment_sale', 'payment_sent', 'payment_sent_number', 'cash_payment_purchase', 'cheque_payment_purchase', 'credit_card_payment_purchase', 'warehouse_name', 'warehouse_sale', 'warehouse_purchase', 'warehouse_return', 'warehouse_purchase_return', 'warehouse_expense', 'start_date', 'end_date'));
    // }
    ...
    public function cashInHand(Request $request)
    {
        ...
        $previous_balance = [
            ...
            'purchase_return' => ReturnPurchase::whereDate('created_at', '<', $start_date)->sum('grand_total'),
            'sale' => Sale::whereDate('created_at', '<', $start_date)->sum('grand_total'),
            'sale_return' => Returns::whereDate('created_at', '<', $start_date)->sum('grand_total'),
            'payment_received' => Payment::whereNotNull('sale_id')->whereDate('created_at', '<', $start_date)->sum('amount'),
            'payment_sent' => Payment::whereNotNull('purchase_id')->whereDate('created_at', '<', $start_date)->sum('amount'),
            'expense' => Expense::whereDate('created_at', '<', $start_date)->sum('amount'),
            'payroll' => Payroll::whereDate('created_at', '<', $start_date)->sum('amount'),
        ];
        ...
        $data = [
            ...
            'purchase_return' => ReturnPurchase::whereBetween('created_at', [$start_date, $end_date])->sum('grand_total'),
            'sale' => Sale::whereBetween('created_at', [$start_date, $end_date])->sum('grand_total'),
            'sale_return' => Returns::whereBetween('created_at', [$start_date, $end_date])->sum('grand_total'),
        ...
        return view('backend.report.cash', compact('previous_balance', 'previous_balance_total', 'data', 'total_current_balance', 'cash', 'start_date', 'end_date'));
    }
        ...
        if (!empty($purchases)) {
            foreach ($purchases as $key => $purchase) {
                ...
                $product_purchase_data = DB::table('purchases')->join('product_purchases', 'purchases.id', '=', 'product_purchases.purchase_id')
                    ->join('products', 'product_purchases.product_id', '=', 'products.id')
                    ->where('purchases.id', $purchase->id)
                    ->select('products.name as product_name', 'products.code as product_code', 'product_purchases.qty', 'product_purchases.purchase_unit_id')
                    ->get();
                ...
                foreach ($product_purchase_data as $product_purchase) {
                    ...
                    $productList[] = [
                        'name' => $product_purchase->product_name,
                        'code' => $product_purchase->product_code ?? '',
                        'qty' => number_format($product_purchase->qty, cache()->get('general_setting')->decimal) . ($unitCode ? ' ' . $unitCode : '')
                    ];
                }

                $totalItems = count($productList);
                if ($totalItems === 0) {
                    $nestedData['product'] = '<span class="text-muted">-</span>';
                } elseif ($totalItems <= 2) {
                    $html = '';
                    foreach ($productList as $p) {
                        $codeSpan = !empty($p['code']) ? ' <span class="text-muted font-weight-normal">[' . htmlspecialchars($p['code']) . ']</span>' : '';
                        $html .= '<span class="badge badge-light border d-block text-left p-1 mb-1" style="font-size: 0.85rem; white-space: normal;">' . htmlspecialchars($p['name']) . $codeSpan . ' <strong class="text-primary">(' . $p['qty'] . ')</strong></span>';
                    }
                    $nestedData['product'] = $html;
                } else {
                    $jsonProducts = htmlspecialchars(json_encode($productList), ENT_QUOTES, 'UTF-8');
                    $firstCode = !empty($productList[0]['code']) ? ' <span class="text-muted font-weight-normal">[' . htmlspecialchars($productList[0]['code']) . ']</span>' : '';
                    $html = '<span class="badge badge-light border d-block text-left p-1 mb-1" style="font-size: 0.85rem; white-space: normal;">' . htmlspecialchars($productList[0]['name']) . $firstCode . ' <strong class="text-primary">(' . $productList[0]['qty'] . ')</strong></span>';
                    $html .= '<button type="button" class="btn btn-outline-info btn-sm view-products-btn py-0 px-2 mt-1" style="font-size: 0.8rem;" data-reference="' . htmlspecialchars($purchase->reference_no) . '" data-products=\'' . $jsonProducts . '\'><i class="fa fa-list"></i> +' . ($totalItems - 1) . ' More (' . $totalItems . ' Items)</button>';
                    $nestedData['product'] = $html;
                }
                $nestedData['grand_total'] = number_format($purchase->grand_total, cache()->get('general_setting')->decimal);
                $nestedData['paid'] = number_format($purchase->paid_amount, cache()->get('general_setting')->decimal);
        ...
        if (!empty($return_purchases)) {
            foreach ($return_purchases as $key => $return) {
                ...
                $product_return_data = DB::table('return_purchases')->join('purchase_product_return', 'return_purchases.id', '=', 'purchase_product_return.return_id')
                    ...
                    ->select('products.name as product_name', 'products.code as product_code', 'purchase_product_return.qty', 'purchase_product_return.purchase_unit_id')
                    ->get();
                ...
                foreach ($product_return_data as $product_return) {
                    ...
                    $productList[] = [
                        'name' => $product_return->product_name,
                        'code' => $product_return->product_code ?? '',
                        'qty' => number_format($product_return->qty, cache()->get('general_setting')->decimal) . ($unitCode ? ' ' . $unitCode : '')
                    ];
                }
                ...
                if ($totalItems === 0) {
                    ...
                    $firstCode = !empty($productList[0]['code']) ? ' <span class="text-muted font-weight-normal">[' . htmlspecialchars($productList[0]['code']) . ']</span>' : '';
                    $html = '<span class="badge badge-light border d-block text-left p-1 mb-1" style="font-size: 0.85rem; white-space: normal;">' . htmlspecialchars($productList[0]['name']) . $firstCode . ' <strong class="text-primary">(' . $productList[0]['qty'] . ')</strong></span>';
                    $html .= '<button type="button" class="btn btn-outline-info btn-sm view-products-btn py-0 px-2 mt-1" style="font-size: 0.8rem;" data-reference="' . htmlspecialchars($return->reference_no) . '" data-products=\'' . $jsonProducts . '\'><i class="fa fa-list"></i> +' . ($totalItems - 1) . ' More (' . $totalItems . ' Items)</button>';
                    $nestedData['product'] = $html;
                }
        ...
        if (!empty($quotations)) {
            foreach ($quotations as $key => $quotation) {
                ...
                $product_quotation_data = DB::table('quotations')->join('product_quotation', 'quotations.id', '=', 'product_quotation.quotation_id')
                    ->join('products', 'product_quotation.product_id', '=', 'products.id')
                    ->where('quotations.id', $quotation->id)
                    ->select('products.name as product_name', 'products.code as product_code', 'product_quotation.qty', 'product_quotation.sale_unit_id')
                    ->get();

                $productList = [];
                foreach ($product_quotation_data as $product_quotation) {
                    if ($product_quotation->sale_unit_id) {
                        $unit_data = DB::table('units')->select('unit_code')->find($product_quotation->sale_unit_id);
                        $unitCode = $unit_data->unit_code ?? '';
                    } else {
                        $unitCode = '';
                    }
                    $productList[] = [
                        ...
                        'code' => $product_quotation->product_code ?? '',
                        'qty' => number_format($product_quotation->qty, cache()->get('general_setting')->decimal) . ($unitCode ? ' ' . $unitCode : '')
                    ];
                }
                ...
                if ($totalItems === 0) {
                    ...
                    $firstCode = !empty($productList[0]['code']) ? ' <span class="text-muted font-weight-normal">[' . htmlspecialchars($productList[0]['code']) . ']</span>' : '';
                    $html = '<span class="badge badge-light border d-block text-left p-1 mb-1" style="font-size: 0.85rem; white-space: normal;">' . htmlspecialchars($productList[0]['name']) . $firstCode . ' <strong class="text-primary">(' . $productList[0]['qty'] . ')</strong></span>';
                    $html .= '<button type="button" class="btn btn-outline-info btn-sm view-products-btn py-0 px-2 mt-1" style="font-size: 0.8rem;" data-reference="' . htmlspecialchars($quotation->reference_no) . '" data-products=\'' . $jsonProducts . '\'><i class="fa fa-list"></i> +' . ($totalItems - 1) . ' More (' . $totalItems . ' Items)</button>';
                    $nestedData['product'] = $html;
                }
                $nestedData['grand_total'] = number_format($quotation->grand_total, cache()->get('general_setting')->decimal);
                if ($quotation->quotation_status == 1) {
    ...
    public function stockCount(Request $request)
    {
        ...
        // If no filter given and stock counts exist, default to the latest stock count so report isn't mixing everything
        if (!$countID && !$start_date && !$end_date && !$warehouse_id) {
            $latestCount = DB::table('stock_counts')->orderBy('id', 'desc')->first();
            if ($latestCount) {
                $countID = $latestCount->id;
            }
        }

        $query = DB::table('stock_count_items')
            ->join('stock_counts', 'stock_counts.id', '=', 'stock_count_items.stock_count_id')
            ->leftJoin('warehouses as item_wh', 'item_wh.id', '=', 'stock_count_items.warehouse_id')
            ->leftJoin('warehouses as count_wh', 'count_wh.id', '=', 'stock_counts.warehouse_id')
            ->leftJoin('product_variants', 'product_variants.item_code', '=', 'stock_count_items.item_code')
            ->leftJoin('products', function($join) {
                $join->on('products.id', '=', 'product_variants.product_id')
                     ->orOn('products.id', '=', 'stock_count_items.product_id');
            })
            ->leftJoin('brands', 'brands.id', '=', 'products.brand_id');

        if ($countID) {
            $query->where('stock_count_items.stock_count_id', $countID);
        } else {
            if ($start_date && $end_date) {
                ...
                $query->where('stock_counts.created_at', '>=', $start_date . ' 00:00:00');
            } elseif ($end_date) {
                $query->where('stock_counts.created_at', '<=', $end_date . ' 23:59:59');
            }
        }

        if ($warehouse_id && $warehouse_id !== 'all') {
            $query->where(function($q) use ($warehouse_id) {
                $q->where('stock_count_items.warehouse_id', $warehouse_id)
                  ->orWhere('stock_counts.warehouse_id', $warehouse_id);
            });
        }

        $rawItems = $query->select(
            'stock_count_items.stock_count_id',
            'stock_counts.created_at as count_date',
            'stock_counts.is_completed',
            'stock_counts.is_resolved',
            'products.id as product_id',
            DB::raw('COALESCE(products.name, "Unknown Product") as product_name'),
            'stock_count_items.item_code',
            DB::raw('COALESCE(brands.title, "No Brand") as brand_name'),
            DB::raw('COALESCE(item_wh.name, count_wh.name, "All Warehouse") as warehouse_name'),
            DB::raw('COALESCE(products.cost + product_variants.additional_cost, products.cost, 0) as unit_cost'),
            DB::raw('COALESCE(products.price + product_variants.additional_price, products.price, 0) as unit_price'),
            DB::raw('SUM(stock_count_items.current_quantity) as current_quantity'),
            DB::raw('SUM(stock_count_items.updated_quantity) as updated_quantity')
        )
        ->groupBy(
            'stock_count_items.stock_count_id',
        ...
        ->orderBy('stock_count_items.stock_count_id', 'desc')
        ->orderBy('products.name', 'asc')
        ->orderBy('stock_count_items.item_code', 'asc')
        ->get();
        ...
        foreach ($rawItems as $item) {
            $currQty = floatval($item->current_quantity);
            $updQty = floatval($item->updated_quantity);
            $cost = floatval($item->unit_cost);
            $price = floatval($item->unit_price);

            $diffQty = $updQty - $currQty;
            $item->diff_quantity = $diffQty;
            $item->diff_cost = $diffQty * $cost;
            $item->diff_price = $diffQty * $price;
            $item->current_total_cost = $currQty * $cost;
            $item->current_total_price = $currQty * $price;
            $item->updated_total_cost = $updQty * $cost;
            $item->updated_total_price = $updQty * $price;

            if ($diffQty == 0) {
                $item->status_type = 'matched';
                $matchedCount++;
                $matchedQty += $updQty;
            } elseif ($diffQty > 0) {
                $item->status_type = 'over';
                $overCount++;
                $overQty += $diffQty;
            } else {
                if ($currQty > 0 && $updQty == 0) {
                    $item->status_type = 'zeroed';
                    $zeroedCount++;
                    $zeroedQty += $currQty;
                    $zeroedCost += ($currQty * $cost);
                    $zeroedRevenue += ($currQty * $price);
                } else {
                    $item->status_type = 'under';
                }
                $underCount++;
                $underQty += abs($diffQty);
            }
            ...
            $totalCurrentQty += $currQty;
            $totalUpdatedQty += $updQty;
            $totalCurrentCost += ($currQty * $cost);
            $totalCurrentRevenue += ($currQty * $price);
            $totalUpdatedCost += ($updQty * $cost);
            $totalUpdatedRevenue += ($updQty * $price);

            $brand = $item->brand_name ?? 'No Brand';
            if (!isset($brandWiseStockCounts[$brand])) {
                $brandWiseStockCounts[$brand] = [
                    'current_qty' => 0,
                    'updated_qty' => 0,
                    'diff_qty' => 0,
                ];
            }
            $brandWiseStockCounts[$brand]['current_qty'] += $currQty;
            $brandWiseStockCounts[$brand]['updated_qty'] += $updQty;
            $brandWiseStockCounts[$brand]['diff_qty'] += $diffQty;
        }

        $totalProducts = count($uniqueProductIds);
        $totalItems = count($rawItems);
        $totalDiffQty = $totalUpdatedQty - $totalCurrentQty;
        $totalDiffCost = $totalUpdatedCost - $totalCurrentCost;
        $totalDiffRevenue = $totalUpdatedRevenue - $totalCurrentRevenue;
        ...
        $stockCountItems = $rawItems;

        return view('backend.report.stock_count', compact(
            'stockCountItems',
            'brandWiseStockCounts',
            'totalProducts',
            'totalItems',
    ...
    public function stockCountRemaining(Request $request)
    {
        ...
        $counted_product_ids = DB::table('stock_count_items')
            ->join('stock_counts', 'stock_counts.id', '=', 'stock_count_items.stock_count_id')
            ->leftJoin('product_variants', 'product_variants.item_code', '=', 'stock_count_items.item_code')
            ->leftJoin('products', 'products.code', '=', 'stock_count_items.item_code')
            ->when($countID, function ($query) use ($countID) {
                return $query->where('stock_count_items.stock_count_id', $countID);
            })
            ->when(!$countID && ($start_date || $end_date), function ($query) use ($start_date, $end_date) {
                if ($start_date && $end_date) {
                ...
                if ($start_date) {
                    return $query->where('stock_counts.created_at', '>=', $start_date . ' 00:00:00');
                }
                if ($end_date) {
                    return $query->where('stock_counts.created_at', '<=', $end_date . ' 23:59:59');
                }
            })
            ->selectRaw('COALESCE(product_variants.product_id, products.id, stock_count_items.product_id) as correct_product_id')
        ...
        $query = Product::where('is_active', 1)
            ->whereNotIn('id', $counted_product_ids);
